...
- **`launcher/bridge/einky_bridge.py`** screen-grabs that framebuffer with `mss`, fits it to 800×480, Floyd–Steinberg dithers to 1 bit, packs it MSB-first (48000 bytes) and POSTs it to `http://<esp>/frame`. It hashes each frame with blake2b and only sends when the hash changes — the e-ink panel can't sustain high refresh rates, so this also rate-limits naturally. A background thread polls `GET /input` every 50 ms and replays each queued button name as a key into the same `DISPLAY` via `xdotool`.
- **`launcher/bridge/esp32/einky_esp32.ino`** runs an `ESPAsyncWebServer` on port 80. `POST /frame` accepts a 48000-byte body, copies it into RAM and triggers a full-window refresh via `GxEPD2`. `GET /test` redraws the boot self-test pattern. `GET /input` returns and clears the queued button events (one name per line). The bit convention is `1 = black, 0 = white` (the bridge inverts PIL's default to match `GxEPD2::drawBitmap`).
...
| Var | Default | Purpose |
|-----|---------|---------|
| `ESP_URL` | `http://192.168.1.50/frame` | Where to POST frames. The input poller derives `<scheme>://<host>/input` from this. |
...
- **`incomplete` 400 from the ESP** — body length didn't match `FRAME_BYTES` (48000). Almost always a `PANEL_W`/`PANEL_H` mismatch in the bridge versus the panel driver chosen in the firmware.
